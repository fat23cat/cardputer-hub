...
## USB and Bluetooth Scope

USB provides power, firmware installation, and diagnostics through the ESP32-S3
USB Serial/JTAG peripheral. The firmware does not enumerate a USB keyboard or
send host-control reports over the cable. There is no active-channel setting.
BLE is the sole implemented HID transport, but remains internal infrastructure
until the later host-control UI is delivered. Use the opt-in
[BLE validation harness](../validation/plan-015-device-harness.md) for hardware
checks; its commands are not normal firmware controls.
