...
commit on `main`. After a push-triggered `main` CI run succeeds, the workflow
looks up the merged pull request and uses its source branch type to decide
whether and how to release the exact validated commit. Direct pushes, manual,
failed, and cancelled CI runs must not create releases.
...
Production versions are generated by CI/CD rather than manually editing version
constants in multiple locations. The first automatic release is `v0.1.0`.
Subsequent versions use the merged pull request's source branch prefix:

| Source branch prefix | Version result |
| --- | --- |
| `major/`, `breaking/` | Increment major; reset minor and patch |
| `feat/`, `minor/` | Increment minor; reset patch |
| `fix/`, `perf/`, `patch/` | Increment patch |
| Any other prefix | Do not create a firmware release |

The calculation starts from the highest permanent semantic-version Git tag,
including tags whose older GitHub Release record has been removed. Commit text
does not determine the version.
...
```text
eligible pull request merge passes the complete main CI workflow
    ↓
full validation
    ↓
version assigned
