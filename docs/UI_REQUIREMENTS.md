...
and Vermilion for Error; the label stays Ink. When a live authenticated Companion
session for the selected host is ready, a compact Leaf diamond appears to the
right of that status row. The diamond is absent in every other Companion state;
Home never shows Companion error, waiting, or off copy. There is no title or footer on Home.
...
ESC CANCEL, plus ENTER NEXT only when the SSID is non-empty, ENTER CONNECT on
the password editor, or ENTER FORGET. The password
